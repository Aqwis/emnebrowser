...
var DatabaseLogic = function () {
	...
	this.getCourseDict = function (callback) {
	    ...
	    http.get("http://data.uio.no/studies/v1/courses", function(res) {
	        res.setEncoding('utf8');
	        res.on('data', function(data) {
	            coursesRaw += data;
	        }).on('end', function() {
	            courseDict = JSON.parse(coursesRaw);
	            fs.writeFile('uio_course_list.json', coursesRaw, function(err) {
	                if(err) {
	                    throw err;
	                }
	            });
	...
	this.getAllCourses = function (courseDict) {
	    ...
	    courseDict.courses.forEach(function(course) {
	        course.code = course.code.replace('/', ''); // sanitize course codes
	        if (fs.existsSync('uio_courses/' + course.code + ".json")) {
	...
	this.getAndSaveCourseInfo = function (courseCode) {
	    ...
	    http.get(encodeURI("http://data.uio.no/studies/v1/course/" + courseCode), function(res) {
	        res.setEncoding('utf8');
	        res.on('data', function(data) {
	            courseDataRaw += data;
	        }).on('end', function() {
	            fs.writeFile('uio_courses/' + courseCode + ".json", courseDataRaw, function(err) {
	                if (err) {
	                    throw err;
	                }
	...
	this.insertCourseData = function (connection, callback) {
	    ...
	    filenames.forEach(function (filename) {
	        try {
	            json = require('../uio_courses/' + filename);
	        } catch (err) {
	        	console.log(err);
